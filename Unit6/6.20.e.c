...
int main()
{
    ...
    printf("Time of %dx game: %d seconds\n", 100, time2 - time1); //time of 100 games

    printf("Ration in first round: %.2f%c\n", (float) winCounter[1] / (winCounter[1] + loseCounter[1]) * 100, '%');
    int ratioInFirstRound = (float) winCounter[1] / (winCounter[1] + loseCounter[1]) * 100;

    int winPossibility = 0, losePossibility = 0;
    for (round = 2 ; round <= 21 ; round++)
        {
            winPossibility += winCounter[round];
            losePossibility += loseCounter[round];
        }
    printf("Ration in other rounds: %.2f%c\n", (float) winPossibility / (winPossibility + losePossibility) * 100, '%');
    int ratioInOtherRounds = (float) winPossibility / (winPossibility + losePossibility) * 100;

    if (ratioInFirstRound > ratioInOtherRounds) //First rounds or continues? In which status the win ratio is bigger?
        printf("Win ration in first round is bigger than if game continues\n");
    else 
        printf("If game continues, win possibility will be bigger\n");
